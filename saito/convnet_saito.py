...
# Pooling
class Pooling:
    def __init__(self, pool_h, pool_w, straide=1, pad=0):
        self.pool_h = pool_h
        self.pool_w = pool_w
        self.stride = stride
        self.pad = pad
    ...
    def forward(self, x):
        ...
        arg_max = np.argmax(col, axis=1) # for backward
        out = np.max(col, axis=1)
        out = out.reshape(N, out_h, out_w, C).transpose(0, 2, 1, 2)
    ...
    def backward(self, dout):
        ...
        dcol = dmax.reshape(dmax.shape[0] * dmas.shape[1] * dmax.shape[2], -1)
